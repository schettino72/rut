...
class Worker:
    ...
    @classmethod
    def read_one(cls, in_stream) -> tuple(int, Any):
...
class WorkerManager:
    ...
    def _write_pack(self, msg_type, payload):
        ...
        transport.write(struct.pack('!BI', msg_type, len(payload)))
...
class Master:
    ...
    async def iter_out_msgs(self) -> Iterator[tuple(WorkerManager, int, Any)]:
